...
interface ImageUrlCheckResponse {
	getImageUrlCheck: {
		divisionCode: string;
		urls: string[];
	}[];
...
export const actions: Actions = {
	checkUrls: async ({ request }): Promise<ServerResponse> => {
		...
		try {
			const response = await client.query<ImageUrlCheckResponse>({
				query,
			...
			if (!response.data.getImageUrlCheck || response.data.getImageUrlCheck.length === 0) {
