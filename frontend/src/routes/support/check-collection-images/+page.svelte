<!--+page.svelte - For Checking Collection Images-->
<script lang="ts">
	...
	import { translateCode, seasonTranslations, divisionTranslations } from '$lib/utils/translations';
		...
		try {
			for (const division of sortedData) {
				...
				for (let i = 0; i < urls.length; i += concurrency) {
					...
					const promises = batch.map(url => checkUrlWithRetry(`${baseUrl}${url}`, divisionCode, signal));
	...
	function exportFailedUrls() {
		...
		link.setAttribute("download", "failed_collection_urls.csv");
